...
const OrderPage = () => {
  ...
  const handleSendCancelReason = (orderId, reason) => {
    console.log("Send cancel for:", orderId, "Reason:", reason);
    // Gọi API huỷ đơn ở đây nếu có
    ...
    <>
      ...
      <div className="p-4 space-y-6 max-w-4xl mx-auto">
        ...
        {filteredOrders.map((order) => {
          ...
          ? order.items.reduce((sum, item) => sum + item.salePrice * item.buyQuantity, 0)
          : 0;
          ...
          return (
            ...
            >
              ...
              <div className="space-y-4 border-b pb-4">
                {Array.isArray(order.items) &&(isExpanded ? order.items : [order.items[0]]).map((item) => (
                  <div
                    key={item.productId}
                ...
                {order.status !== "Completed" &&
                  order.status !== "Cancelled" && (
                    <>
                      <button
                        onClick={() => {
                          if (cancelOrderId === order.orderId) {
                            setCancelOrderId(null);
                            setSelectedReason("");
                          } else {
