          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              ...
              <div className="flex items-center gap-3 mb-3">
                <span className="line-through text-gray-500 text-lg">${parseFloat(product.oldprice).toLocaleString()}</span>
                <span className="text-red-600 text-2xl font-semibold">${product.saleprice}</span>
              </div>

              <div className="flex items-center gap-3 mb-4">
                <span className="text-sm text-gray-600">Quantity:</span>
                <button onClick={() => setBuyQuantity(Math.max(1, buyQuantity - 1))} className="border px-3 py-1 rounded">-</button>
                <span className="px-2">{buyQuantity}</span>
                <button onClick={() => setBuyQuantity(buyQuantity + 1 <= 99 ? buyQuantity + 1 : buyQuantity)} className="border px-3 py-1 rounded">+</button>
              </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex gap-4">
                <button onClick={handleAddToCart} className="flex-1 px-6 py-3 border border-red-600 text-red-600 rounded hover:bg-red-100">Add to cart</button>
                <button onClick={handleBuyNow} className="flex-1 px-6 py-3 bg-red-600 text-white rounded hover:bg-red-700">Buy now</button>
              </div>
            </div>
          </div>
        </div>
